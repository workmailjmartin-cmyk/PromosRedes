document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA DE DURACIÓN (PRIORIDAD HOTELES) ---
    function getNoches(pkg) {
        ...
        // 1. Prioridad: Suma de noches de hoteles
        let totalNochesHoteles = 0;
        let hayHotel = false;
        servicios.forEach(s => {
            if (s.tipo === 'hotel' && s.noches) {
                totalNochesHoteles += parseInt(s.noches) || 0;
                hayHotel = true;
            }
        });
        if (hayHotel && totalNochesHoteles > 0) return totalNochesHoteles;
        ...
        // 3. Fallback: Cálculo por fechas (original)
        ...
        if (Array.isArray(servicios)) {
            servicios.forEach(s => {
                if(s.tipo === 'aereo') {
                    ...
                    // CAMBIO: Mostrar noches e ingreso
    ...
    function renderServiciosClienteHTML(rawJson) { 
        ...
        s.forEach(x=>{ 
            ...
            if(x.tipo==='aereo'){
                ...
                // CAMBIO: Mostrar noches e ingreso en modal
                let detalle = [];
                if(x.noches) detalle.push(`${x.noches} Noches`);
                if(x.checkin) detalle.push(`Ingreso: ${formatDateAR(x.checkin)}`);
                if(detalle.length > 0) l.push(detalle.join(' | '));
            } 
            else if(x.tipo==='traslado'){i='🚕';t='TRASLADO';l.push(`${x.tipo_trf}`);} 
            else if(x.tipo==='seguro'){
                i='🛡️';t='SEGURO';
                // CAMBIO: Mostrar proveedor
    ...
    // --- SEGURIDAD: COLA Y MUTEX (Agregado al código base) ---
