document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===========================================================
    // 3. LÓGICA DE NEGOCIO (CALCULOS, TEXTOS)
    // ===========================================================
    ...
    // ===========================================================
    // 7. INTERFAZ Y EVENTOS (FORMULARIO)
    // ===========================================================
    ...
    window.setStars = (id, count) => {
        ...
        if(container && input) { input.value = count; const spans = container.querySelectorAll('span'); spans.forEach((span, idx) => { if(idx < count) span.classList.add('filled'); else span.classList.remove('filled'); }); }
    };
    ...
    window.calcularNoches = (id) => { 
        const c=document.querySelector(`.servicio-card[data-id="${id}"]`); if(!c)return; const i=c.querySelector('input[name="checkin"]'), o=c.querySelector('input[name="checkout"]'); 
        if(i&&o&&i.value&&o.value){ const d1=new Date(i.value), d2=new Date(o.value); const diff = (d2>d1)?Math.ceil((d2-d1)/86400000):0; const inputN = document.getElementById(`noches-${id}`); if(inputN) inputN.value = diff; } 
    };
    ...
    function agregarModuloServicio(tipo, data = null) {
        ...
        if (!data) { if (hasExclusive && tipo !== 'adicional') return window.showAlert("⛔ Solo Adicionales.", "error"); if ((tipo === 'bus' || tipo === 'crucero') && existingServices.length > 0) return window.showAlert("⛔ Exclusivo.", "error"); }
    ...
    // ===========================================================
    // 8. EVENTOS Y AUTH (DEFINIDOS AL FINAL PARA EVITAR ERRORES)
    // ===========================================================
    if(dom.logoImg) dom.logoImg.addEventListener('click', () => { if(currentUser) showView('search'); else window.location.reload(); });
        ...
        if (u) {
            try {
                ...
                if (doc.exists) {
                    ...
                    configureUIByRole(); // AHORA SÍ ESTÁ DEFINIDA
    ...
    dom.uploadForm.addEventListener('submit', async (e) => {
        ...
        try { 
            ...
            await fetchAndLoadPackages();
            showView('search');
        } catch(e) { window.showAlert(e.message || "Error al guardar.", 'error'); }
    });
