document.addEventListener('DOMContentLoaded', () => {
    ...
    const db = firebase.firestore(); // Base de datos de usuarios
    const provider = new firebase.auth.GoogleAuthProvider();

    let currentUser = null;
    let currentPerms = {}; // Aquí guardaremos los permisos del usuario actual
    let allPackages = [];
    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            // Verificamos permisos en Firestore
            const permisos = await obtenerPermisos(user.email);
            
            if (permisos.acceso || user.email === SUPER_ADMIN) {
                currentUser = user;
                currentPerms = permisos; // Guardamos permisos en memoria
                ...
                // UI: Configurar Botones según Permisos
                setupNavPermissions();
                ...
                dom.authError.textContent = 'Tu usuario no está autorizado. Contacta al administrador.';
                auth.signOut();
            }
        } else {
            currentUser = null;
    ...
    async function obtenerPermisos(email) {
        if (email === SUPER_ADMIN) return { acceso: true, paquetes: true, usuarios: true }; // God Mode
        try {
            const docRef = db.collection('usuarios').doc(email);
            const doc = await docRef.get();
            if (doc.exists) return { acceso: true, ...doc.data() };
        } catch (e) { console.error("Error leyendo permisos", e); }
        return { acceso: false };
    }

    function setupNavPermissions() {
        // Mostrar botones solo si tiene permiso o es Super Admin
        dom.navUpload.style.display = (currentPerms.paquetes || currentUser.email === SUPER_ADMIN) ? 'inline-block' : 'none';
    ...
    async function cargarTablaUsuarios() {
        ...
        snapshot.forEach(doc => {
            ...
            const email = doc.id;
            let badges = '';
            if (data.paquetes) badges += '<span class="badge-permiso bg-blue">Paquetes</span> ';
            if (data.usuarios) badges += '<span class="badge-permiso bg-orange">Admin</span> ';
            
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${email}</td>
                <td>${badges || '<small style="color:#999">Solo lectura</small>'}</td>
                <td>
                    <button class="btn btn-secundario btn-sm" onclick="window.editarUsuario('${email}', ${data.paquetes}, ${data.usuarios})">Editar</button>
    ...
    dom.formUsuario.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            alert(`Usuario ${email} actualizado.`);
    ...
    window.editarUsuario = (email, p, u) => {
        dom.inputAdminEmail.value = email;
        dom.inputAdminEmail.disabled = true; 
        dom.checkPermPaquetes.checked = !!p; dom.checkPermUsuarios.checked = !!u;
        dom.modalUsuario.style.display = 'flex';
    };
    window.borrarUsuario = async (email) => {
        if(!confirm(`¿Quitar acceso a ${email}?`)) return;
        try { await db.collection('usuarios').doc(email).delete(); cargarTablaUsuarios(); } catch(e) { alert("Error: "+e.message); }
    };
    ...
    // =========================================================
    // 6. LÓGICA DE CARGA DE PAQUETES (VALIDADA)
    // =========================================================
