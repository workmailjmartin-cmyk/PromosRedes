document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        calendarContainer: document.getElementById('calendar-container'), calendarGrid: document.getElementById('calendar-grid'),
        btnToggleCalendar: document.getElementById('btn-toggle-calendar')
    };

    // ===========================================================
    // 2. FUNCIONES BÁSICAS (IMPORTANTE: ESTÁN PRIMERO)
    // ===========================================================
    ...
    // ===========================================================
    // 3. FUNCIONES DE DATOS (AQUÍ ESTABA EL ERROR ANTES)
    // ===========================================================
        ...
        try {
            ...
            days.forEach((dayName, index) => {
                ...
                if (userData && (userData.rol === 'admin' || userData.rol === 'editor')) {
                    editBtn = `<button class="btn-edit-day" onclick="editCalendarDay('${dayKey}', '${dayName}')" title="Editar">✏️</button>`;
                }
    ...
    window.editCalendarDay = async (dayKey, dayName) => {
        ...
        const newVal = prompt(`Editar ${dayName}:`, currentVal);
        if (newVal !== null) {
            showLoader(true, "Guardando...");
            try {
                await db.collection('config').doc('weekly_calendar').set({ [dayKey]: newVal }, { merge: true });
                await loadCalendar();
                showLoader(false);
            } catch (e) { showLoader(false); window.showAlert("Error al guardar.", "error"); }
        }
    };

    // ===========================================================
    // 5. FUNCIONES DE PAQUETE (VISUAL Y TEXTO)
    // ===========================================================
    ...
    // ===========================================================
    // 6. RED & CARGA (DEFINIDAS ANTES DE USAR)
    // ===========================================================
    ...
    function configureUIByRole() {
        ...
        if(selectPromo) {
            selectPromo.innerHTML = rol === 'usuario' ? '<option value="Solo X Hoy">Solo X Hoy</option><option value="FEED">FEED (Requiere Aprobación)</option>' : '<option value="FEED">FEED</option><option value="Solo X Hoy">Solo X Hoy</option><option value="ADS">ADS</option>';
        }
        updatePendingBadge(); 
    }

    // ===========================================================
    // 7. INICIALIZACIÓN (LISTENER PRINCIPAL)
    // ===========================================================
        ...
        if (u) {
            try {
                ...
                if (doc.exists) {
                    ...
                    // IMPORTANTE: Primero UI, luego datos
                    configureUIByRole(); 
                    await loadCalendar();
    ...
    // ===========================================================
    // 8. EVENTOS DE UI (LOGO, BOTONES)
    // ===========================================================
    if(dom.logoImg) {
        dom.logoImg.addEventListener('click', () => {
            if(currentUser) {
                // RESET SUAVE: Limpiar filtros y volver a buscar
