document.addEventListener('DOMContentLoaded', () => {
    ...
    // Referencias DOM Seguras (Usamos getElementById directo en la lógica para evitar nulls al inicio)
    const dom = {
    ...
    window.showAlert = (message, type = 'error') => { return new Promise((resolve) => { showLoader(false); const overlay = document.getElementById('custom-alert-overlay'); const title = document.getElementById('custom-alert-title'); const msg = document.getElementById('custom-alert-message'); const icon = document.getElementById('custom-alert-icon'); const btn = document.getElementById('custom-alert-btn'); const btnCancel = document.getElementById('custom-alert-cancel'); if(overlay) { if (type === 'success') { title.innerText = '¡Éxito!'; title.style.color = '#4caf50'; icon.innerHTML = '✅'; } else if (type === 'info') { title.innerText = 'Información'; title.style.color = '#3498db'; icon.innerHTML = 'ℹ️'; } else { title.innerText = 'Atención'; title.style.color = '#ef5a1a'; icon.innerHTML = '⚠️'; } msg.innerText = message; overlay.style.display = 'flex'; btn.onclick = () => { overlay.style.display = 'none'; resolve(); }; } else { alert(message); resolve(); } }); };
    
    window.showConfirm = (message) => { return new Promise((resolve) => { showLoader(false); const overlay = document.getElementById('custom-alert-overlay'); const title = document.getElementById('custom-alert-title'); const msg = document.getElementById('custom-alert-message'); const icon = document.getElementById('custom-alert-icon'); const btnOk = document.getElementById('custom-alert-btn'); const btnCancel = document.getElementById('custom-alert-cancel'); if(overlay && btnCancel) { title.innerText = 'Confirmación'; title.style.color = '#11173d'; icon.innerHTML = '❓'; msg.innerText = message; btnCancel.style.display = 'inline-block'; overlay.style.display = 'flex'; btnOk.onclick = () => { overlay.style.display = 'none'; resolve(true); }; btnCancel.onclick = () => { overlay.style.display = 'none'; resolve(false); }; } else { resolve(confirm(message)); } }); };

    // --- NAVEGACIÓN SEGURA ---
    function showView(viewName) {
        // Ocultar todas
        Object.values(dom.views).forEach(v => { if(v) v.classList.remove('active'); });
        Object.values(dom.nav).forEach(n => { if(n) n.classList.remove('active'); });
    ...
    window.confirmDeleteUser = async (e) => { if(await window.showConfirm("¿Eliminar usuario?")) try { showLoader(true); await db.collection('usuarios').doc(e).delete(); await loadUsersList(); showLoader(false); } catch(x){alert('Error');} };

    // PAQUETES (MODAL ACTIONS)
    ...
    window.approvePackage = async (pkg) => { 
        if (!await window.showConfirm("¿Aprobar publicación?")) return; 
        showLoader(true); 
        try { 
    ...
    window.startEditing = async (pkg) => { 
        if (!await window.showConfirm("Editar paquete...")) return; 
    ...
    window.openModal = function(pkg) {
        if (!dom.modal || !dom.modalBody) return console.error("Falta modal en HTML");
        ...
        try {
            ...
            days.forEach((dayName, index) => {
                ...
                if (userData && (userData.rol === 'admin' || userData.rol === 'editor')) {
                    editBtn = `<button class="btn-edit-day" onclick="window.openCalendarModal('${dayKey}', '${dayName}')" title="Editar">✏️</button>`;
                }
    ...
    // ===========================================================
    // 5. LISTENERS SEGUROS (NO FALLAN SI FALTA UN ELEMENTO)
    // ===========================================================
