document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===========================================================
    // 3. COMUNICACIÓN SEGURA Y COLA (MUTEX)
    // ===========================================================
    async function secureFetch(url, body) {
        if (!currentUser) throw new Error('No auth');
        // Si es búsqueda, va directo. Si es subida (API_URL_UPLOAD), usa la cola.
    ...
    async function _doFetch(url, body) {
        ...
        if (j.error || j.status === 'error' || (Array.isArray(j) && j.length === 0 && url === API_URL_UPLOAD)) throw new Error(j.message || "Error procesando en el servidor.");
        return j;
    }
    ...
    async function uploadWithMutex(url, body) {
        ...
        while(!acquired && attempts < 20) {
            try {
                await db.runTransaction(async (t) => {
                    ...
                    if (!doc.exists) {
                        // Si no existe el archivo de bloqueo, lo crea y lo toma
                        t.set(lockRef, { locked: true, user: currentUser.email, timestamp: now });
                        return;
                    }

                    const data = doc.data();
                    // Si está bloqueado hace menos de 15s, fallar para esperar
        ...
        if(!acquired) throw new Error("El sistema está saturado (muchos usuarios cargando). Por favor intenta en 1 minuto.");
        ...
        try {
            ...
            // Liberar siempre al terminar
            await lockRef.set({ locked: false });
        }
    }

    // ===========================================================
    // 4. GENERADORES DE CONTENIDO (Tarjetas y Textos)
    // ===========================================================
    
    // Texto para copiar a WhatsApp
    ...
    // Renderizado HTML de servicios (Para el Modal y Tarjetas)
    function renderServiciosClienteHTML(rawJson) { 
        ...
        s.forEach(x=>{ 
            ...
            if(x.tipo==='aereo'){ i='✈️';t='AÉREO'; l.push(`<b>${x.aerolinea}</b>`); l.push(`${formatDateAR(x.fecha_aereo)}${x.fecha_regreso?` - ${formatDateAR(x.fecha_regreso)}`:''}`); let es = (x.escalas == 0 || x.escalas == '0') ? "Directo" : (x.escalas == 1 ? "1 Escala" : `${x.escalas} Escalas`); l.push(`🔄 ${es} | 🧳 ${x.tipo_equipaje || '-'}`);} 
                ...
                i='🛡️';t='SEGURO';
                // CAMBIO: Muestra proveedor en negrita
    ...
    // ===========================================================
    // 5. LÓGICA DE FILTRADO Y CARGA (Aquí estaba el error antes)
    // ===========================================================
    async function fetchAndLoadPackages() { 
        ...
        try { 
            ...
            applyFilters();  // ESTA ES LA FUNCIÓN QUE FALTABA
            updatePendingBadge(); 
    ...
    // ===========================================================
    // 8. FORMULARIO DE CARGA DINÁMICO
    // ===========================================================
