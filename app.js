document.addEventListener('DOMContentLoaded', () => {
    ...
    function generarTextoPresupuesto(pkg) {
        ...
        if (Array.isArray(servicios)) {
            servicios.forEach(s => {
                if(s.tipo === 'aereo') {
                    ...
                    let escalasTxt = (eIda === eVuelta) ? formatEscalasTexto(eIda) : `IDA: ${formatEscalasTexto(eIda)} | REGRESO: ${formatEscalasTexto(eVuelta)}`;
        ...
        texto += tieneSeguro ? `Asistencia al viajero es requisito obligatorio en la mayoría de los destinos internacionales` : `Asistencia al viajero no incluida. Puede añadirse al reservar o más adelante. Es requisito obligatorio en la mayoría de los destinos internacionales`;
        return texto;
    }

    window.copiarPresupuesto = (pkg) => {
        const texto = generarTextoPresupuesto(pkg);
        navigator.clipboard.writeText(texto).then(() => window.showAlert("✅ ¡Presupuesto copiado!", "success")).catch(() => window.showAlert("Error al copiar texto.", "error"));
    };
    ...
    function renderServiciosClienteHTML(rawJson) { 
        ...
        s.forEach(x=>{ 
            ...
            if(x.tipo==='aereo'){
                ...
                let escalasTxt = (eIda === eVuelta) ? formatEscalasTexto(eIda) : `<b>IDA:</b> ${formatEscalasTexto(eIda)} | <b>REG:</b> ${formatEscalasTexto(eVuelta)}`;
    ...
    async function _doFetch(url, body) {
        ...
        if (jsonResponse.error || jsonResponse.status === 'error' || (Array.isArray(jsonResponse) && jsonResponse.length === 0 && url === API_URL_UPLOAD)) throw new Error(jsonResponse.message || "Error procesando en n8n.");
        return jsonResponse;
    }
    ...
    window.setStars = (id, count) => {
        ...
        if(container && input) {
            ...
            spans.forEach((span, idx) => { if(idx < count) span.classList.add('filled'); else span.classList.remove('filled'); });
        }
    };
    ...
    function agregarModuloServicio(tipo, data = null) {
        ...
        if (!data) { 
            if (hasExclusive && tipo !== 'adicional') return window.showAlert("⛔ Ya hay un servicio exclusivo.", "error"); 
        ...
        if(data){ 
            div.querySelectorAll('input, select, textarea').forEach(input => { 
                if (data[input.name] !== undefined) { 
                    if (input.type === 'checkbox') { input.checked = data[input.name]; if (input.name === 'bus_alojamiento') input.dispatchEvent(new Event('change')); } 
                    else if (input.type === 'hidden') { if(input.name === 'hotel_estrellas') { window.setStars(id, data[input.name]); } else { const counter = input.parentElement.querySelector('.counter-value'); if(counter) counter.innerText = data[input.name]; input.value = data[input.name]; } } 
                    else { input.value = data[input.name]; if (input.name === 'checkin' || input.name === 'checkout') window.calcularNoches(id); } 
                }
                if (input.name === 'escalas_ida' && data['escalas'] !== undefined && data['escalas_ida'] === undefined) { const counter = input.parentElement.querySelector('.counter-value'); if(counter) counter.innerText = data['escalas']; input.value = data['escalas']; }
                if (input.name === 'escalas_vuelta' && data['escalas'] !== undefined && data['escalas_vuelta'] === undefined) { const counter = input.parentElement.querySelector('.counter-value'); if(counter) counter.innerText = data['escalas']; input.value = data['escalas']; }
            }); 
        }
    }
    ...
    window.calcularNoches = (id) => { const c=document.querySelector(`.servicio-card[data-id="${id}"]`); if(!c)return; const i=c.querySelector('input[name="checkin"]'), o=c.querySelector('input[name="checkout"]'); if(i&&o&&i.value&&o.value){ const d1=new Date(i.value), d2=new Date(o.value); const diff = (d2>d1)?Math.ceil((d2-d1)/86400000):0; const inputN = document.getElementById(`noches-${id}`); if(inputN) inputN.value = diff; } };
    
    window.calcularTotal = () => { let t=0; document.querySelectorAll('.input-costo').forEach(i=>t+=parseFloat(i.value)||0); dom.inputCostoTotal.value = t; const tarifaSugerida = Math.round(t * 1.185); dom.inputTarifaTotal.value = tarifaSugerida; dom.inputTarifaTotal.dispatchEvent(new Event('input')); };
    ...
    if(dom.inputTarifaTotal) {
        dom.inputTarifaTotal.addEventListener('input', () => {
            ...
            if(inputPersona) inputPersona.value = formatMoney(porPersona);
        });
    }
    ...
    dom.uploadForm.addEventListener('submit', async (e) => {
        ...
        let creadorFinal; if (isEditingId && originalCreator) { creadorFinal = originalCreator; } else { creadorFinal = userData.franquicia || 'Desconocido'; }
