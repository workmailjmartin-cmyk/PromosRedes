// ===========================================================
// FUNCIONES GLOBALES (FUERA DE TODO PARA QUE NO FALLEN NUNCA)
// ===========================================================
...
function renderServiciosClienteHTML(rawJson) { 
    ...
    s.forEach(x=>{ 
        ...
        if(x.tipo==='aereo'){
            ...
            let escalasTxt = "Directo"; if (x.escalas > 0) { escalasTxt = (x.escalas == 1) ? "1 Escala" : `${x.escalas} Escalas`; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showLoader = (show, text = null) => { if(dom.loader){ dom.loader.style.display = show ? 'flex' : 'none'; let p = dom.loader.querySelector('p'); if(p && text) p.innerText = text; }};
    window.showAlert = (message, type = 'error') => { return new Promise((resolve) => { showLoader(false); const overlay = document.getElementById('custom-alert-overlay'); const title = document.getElementById('custom-alert-title'); const msg = document.getElementById('custom-alert-message'); const icon = document.getElementById('custom-alert-icon'); const btn = document.getElementById('custom-alert-btn'); if(overlay){ title.innerText = type==='success'?'¡Éxito!':(type==='info'?'Información':'Atención'); title.style.color = type==='success'?'#4caf50':(type==='info'?'#3498db':'#ef5a1a'); msg.innerText = message; overlay.style.display = 'flex'; btn.onclick = () => { overlay.style.display = 'none'; resolve(); }; } else { alert(message); resolve(); } }); };
    window.showConfirm = (message) => { return new Promise((resolve) => { showLoader(false); const overlay = document.getElementById('custom-alert-overlay'); const title = document.getElementById('custom-alert-title'); const msg = document.getElementById('custom-alert-message'); const btnOk = document.getElementById('custom-alert-btn'); const btnCancel = document.getElementById('custom-alert-cancel'); if(overlay && btnCancel){ title.innerText = 'Confirmación'; msg.innerText = message; btnCancel.style.display = 'inline-block'; overlay.style.display = 'flex'; btnOk.onclick = () => { overlay.style.display = 'none'; resolve(true); }; btnCancel.onclick = () => { overlay.style.display = 'none'; resolve(false); }; } else { resolve(confirm(message)); } }); };
        ...
        try {
            ...
            days.forEach((dayName, index) => {
                ...
                if (userData && (userData.rol === 'admin' || userData.rol === 'editor')) {
                    // IMPORTANTE: stopPropagation para que no "traspase" el clic
                ...
                // Si es admin y toca la tarjeta (no solo el lapiz), también abre
    ...
    function renderCards(list, targetGrid = dom.grid) {
        ...
        list.forEach(pkg => {
            ...
            // EL CLICK LLAMA A WINDOW.OPENMODAL
    ...
    // HELPERS GLOBALES (LINKED WINDOW)
