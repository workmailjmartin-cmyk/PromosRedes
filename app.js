document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- COLA / MUTEX (Sistema de espera para carga simultánea) ---
    async function secureFetch(url, body) {
        if (!currentUser) throw new Error('No auth');
        // Si es búsqueda, va directo. Si es subida (API_URL_UPLOAD), usa cola.
    ...
    async function _doFetch(url, body) {
        ...
        // Validación extra para evitar falsos positivos de n8n
    ...
    // --- CARGA INICIAL Y FILTROS (LAS FUNCIONES QUE FALTABAN) ---
    ...
    // Aquí es donde llamamos a applyFilters, ahora sí existe la función
