document.addEventListener('DOMContentLoaded', () => {
    ...
    // FECHAS (Aquí para evitar errores)
    ...
    // ===========================================================
    // 3. FUNCIONES CORE (PAQUETES, FILTROS, RENDER)
    // ===========================================================
    ...
    function getSummaryIcons(pkg) { 
        let s = []; try { s = typeof pkg.servicios === 'string' ? JSON.parse(pkg.servicios) : pkg.servicios; } catch(e) {} if (!Array.isArray(s)) return ''; 
    ...
    // --- AQUÍ ESTÁ EL ARREGLO: applyFilters definido ANTES de usarse ---
    ...
    window.openCalendarModal = async (dayKey, dayName) => { currentEditingDayKey = dayKey; dom.modalCalendarTitle.innerText = `Editar ${dayName}`; try { const doc = await db.collection('config').doc('weekly_calendar').get(); const currentVal = doc.exists ? (doc.data()[dayKey] || "") : ""; dom.calendarInput.value = currentVal; dom.modalCalendar.classList.add('active'); } catch(e) { window.showAlert("Error al cargar.", "error"); } };
    ...
    dom.uploadForm.addEventListener('submit', async (e) => {
        ...
        try { await secureFetch(API_URL_UPLOAD, payload); await window.showAlert(status === 'pending' ? 'Enviado a revisión.' : 'Guardado correctamente.', 'success'); await softReload(); } catch(e) { window.showAlert(e.message || "Error al guardar.", 'error'); }
    });
