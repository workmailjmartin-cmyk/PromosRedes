document.addEventListener('DOMContentLoaded', () => {
    ...
    window.calcularNoches = (id) => { const c=document.querySelector(`.servicio-card[data-id="${id}"]`); if(!c)return; const i=c.querySelector('input[name="checkin"]'), o=c.querySelector('input[name="checkout"]'); if(i&&o&&i.value&&o.value){ const d1=new Date(i.value), d2=new Date(o.value); const d = (d2>d1)?Math.ceil((d2-d1)/86400000):0; const inN = document.getElementById(`noches-${id}`); if(inN) inN.value = d; } };
    ...
    // ===========================================================
    // 3. LOGICA DE NEGOCIO Y VISUALIZACION
    // ===========================================================
    ...
    function getNoches(pkg) {
        let s = []; try { const raw = pkg['servicios'] || pkg['item.servicios']; s = typeof raw === 'string' ? JSON.parse(raw) : raw; } catch(e) {}
        if(!Array.isArray(s)) return 0;
        let th = 0; let hh = false;
        s.forEach(x => { if (x.tipo === 'hotel' && x.noches) { th += parseInt(x.noches) || 0; hh = true; } });
        if(hh && th > 0) return th;
        const b = s.find(x => x.tipo === 'bus'); if (b && b.bus_noches) return parseInt(b.bus_noches);
        const c = s.find(x => x.tipo === 'crucero'); if (c && c.crucero_noches) return parseInt(c.crucero_noches);
        if(!pkg['fecha_salida']) return 0;
        let f = pkg['fecha_salida']; if(f.includes('/')) f = f.split('/').reverse().join('-');
        const start = new Date(f + 'T00:00:00'); let max = new Date(start); let hd = false;
        s.forEach(x => { if(x.tipo==='hotel'&&x.checkout){ const d=new Date(x.checkout+'T00:00:00'); if(d>max){max=d; hd=true;} } if(x.tipo==='aereo'&&x.fecha_regreso){ const d=new Date(x.fecha_regreso+'T00:00:00'); if(d>max){max=d; hd=true;} } });
        return hd ? Math.ceil((max - start) / 86400000) : 0;
    }
    ...
    function generarTextoPresupuesto(pkg) {
        const noches = getNoches(pkg); const tarifaDoble = Math.round((parseFloat(pkg['tarifa']) || 0) / 2);
        let s = []; try { s = typeof pkg.servicios === 'string' ? JSON.parse(pkg.servicios) : pkg.servicios; } catch(e) {}
    ...
    function renderServiciosClienteHTML(raw) { 
        ...
        s.forEach(x=>{ let i='🔹',t='',l=[]; 
            if(x.tipo==='aereo'){ i='✈️';t='AÉREO'; l.push(`<b>${x.aerolinea}</b>`); l.push(`${formatDateAR(x.fecha_aereo)}${x.fecha_regreso?` - ${formatDateAR(x.fecha_regreso)}`:''}`); l.push(`🧳 ${x.tipo_equipaje || '-'}`);} 
            else if(x.tipo==='hotel'){ i='🏨';t='HOTEL'; let st=''; if(x.hotel_estrellas) for(let k=0;k<x.hotel_estrellas;k++) st+='⭐'; l.push(`<b>${x.hotel_nombre}</b> <span style="color:#ef5a1a;">${st}</span>`); l.push(`(${x.regimen})`); if(x.hotel_link) l.push(`<a href="${x.hotel_link}" target="_blank" style="color:#ef5a1a;font-weight:bold;">📍 Ubicación</a>`);} 
    ...
    window.openModal = function(pkg) {
        ...
        if (canEdit) {
            const btnApprove = (userData.rol === 'admin' || userData.rol === 'editor') && pkg.status === 'pending' ? `<button class="btn btn-primario" onclick='window.approvePackage(${JSON.stringify(pkg).replace(/'/g, "&#39;")})' style="background:#2ecc71;">✅ Aprobar</button>` : '';
            adminTools = `<div class="modal-tools" style="position: absolute; top: 20px; right: 70px; display:flex; gap:10px;">${btnApprove}<button class="btn btn-secundario" onclick='window.startEditing(${JSON.stringify(pkg).replace(/'/g, "&#39;")})'>✏️ Editar</button><button class="btn btn-secundario" onclick='window.deletePackage(${JSON.stringify(pkg).replace(/'/g, "&#39;")})' style="background:#e74c3c; color:white;">🗑️ Borrar</button></div>`;
        }

        dom.modalBody.innerHTML = `${adminTools}
            <div class="modal-detalle-header" style="display:block; padding-bottom:25px;"><h2 style="margin:0;color:#11173d;">${pkg.destino}</h2><div style="margin-top:5px;"><span style="background-color:#56DDE0;color:#11173d;padding:4px 12px;border-radius:20px;font-size:0.8em;">${pkg.tipo_promo}</span></div></div>
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 20px; padding: 20px;">
                <div><h3 style="color:#11173d;">Itinerario</h3>${htmlClient}</div>
                <div style="background:#f9fbfd; padding:15px; border-radius:8px;">
                    <button class="btn" onclick='window.copiarPresupuesto(${JSON.stringify(pkg).replace(/'/g, "&#39;")})' style="width:100%;margin-bottom:10px;background:#34495e;color:white;">📋 Copiar</button>
                    <p><b>📅 Salida:</b> ${formatDateAR(pkg.fecha_salida)}</p><p><b>📍 Desde:</b> ${pkg.salida}</p>
                    <hr><h4>Costos Internos</h4>${htmlCost}
                </div>
            </div>
            <div style="background:#11173d; color:white; padding:15px; display:flex; justify-content:space-between; align-items:center;">
                <div><small>Costo Total</small><br><b>$${formatMoney(pkg.costos_proveedor)}</b></div>
                <div><small>Venta</small><br><b style="color:#ef5a1a;">$${formatMoney(pkg.tarifa)}</b></div>
                <div><small>x Pax (Doble)</small><br><b style="color:#4caf50;">$${formatMoney(tarifaDoble)}</b></div>
            </div>`;
        dom.modal.classList.add('active');
        dom.modal.style.display = 'flex';
    };

    window.deletePackage = async (pkg) => { if (!await window.showConfirm("¿Eliminar?")) return; showLoader(true); try { const id = pkg.id_paquete || pkg.id || pkg['item.id']; await secureFetch(API_URL_UPLOAD, { action_type: 'delete', id_paquete: id, status: 'deleted' }); await window.showAlert("Eliminado.", "success"); if(dom.modal) dom.modal.style.display='none'; await softReload(); } catch (e) { window.showAlert("Error.", "error"); } };
    window.approvePackage = async (pkg) => { if (!await window.showConfirm("¿Aprobar?")) return; showLoader(true); try { let payload = JSON.parse(JSON.stringify(pkg)); payload.status = 'approved'; payload.action_type = 'edit'; await secureFetch(API_URL_UPLOAD, payload); await window.showAlert("Aprobado.", "success"); if(dom.modal) dom.modal.style.display='none'; await softReload(); } catch (e) { window.showAlert("Error.", "error"); } };
    window.startEditing = async (pkg) => { 
        if (!await window.showConfirm("¿Editar?")) return; 
        ...
        let f = pkg.fecha_salida; if(f && f.includes('/')) f = f.split('/').reverse().join('-'); dom.inputFechaViaje.value = f;
        ...
        window.calcularTotal(); if(dom.modal) dom.modal.style.display='none'; showView('upload'); window.scrollTo(0,0); 
    };

    // ===========================================================
    // 6. CARGA DE DATOS (DATA FETCHING)
    // ===========================================================
    ...
    async function fetchAndLoadPackages() { showLoader(true); try { let d = await secureFetch(API_URL_SEARCH, {}); if (typeof d === 'string') d = JSON.parse(d); allPackages = d; uniquePackages = processPackageHistory(allPackages); populateFranchiseFilter(uniquePackages); applyFilters(); updatePendingBadge(); } catch(e){ console.error(e); } showLoader(false); }
    ...
    function populateFranchiseFilter(packages) { const s = dom.filtroCreador; if(!s) return; const v = s.value; const c = [...new Set(packages.map(p => p.creador).filter(Boolean))]; s.innerHTML = '<option value="">Todas</option>'; c.sort().forEach(x => { const o = document.createElement('option'); o.value = x; o.innerText = x; s.appendChild(o); }); s.value = v; }
    function updatePendingBadge() { const b = dom.badgeGestion; if (!b) return; if (!userData || (userData.rol !== 'admin' && userData.rol !== 'editor')) { b.style.display = 'none'; return; } const c = uniquePackages.filter(p => p.status === 'pending').length; if (c > 0) { b.innerText = c; b.style.display = 'inline-block'; } else { b.style.display = 'none'; } }
    ...
    function applyFilters() {
        ...
        let res = uniquePackages.filter(p => { const mD = !fDestino || (p.destino && p.destino.toLowerCase().includes(fDestino)); const mC = !fCreador || (p.creador && p.creador === fCreador); const mP = !fPromo || (p.tipo_promo && p.tipo_promo === fPromo); if (!mD || !mC || !mP) return false; const isOwn = p.editor_email === currentUser.email; const isPen = p.status === 'pending'; if (isPen && !isOwn && userData.rol !== 'admin' && userData.rol !== 'editor') return false; return true; });
        ...
        renderCards(res); if (userData && (userData.rol === 'admin' || userData.rol === 'editor')) { const pen = uniquePackages.filter(p => p.status === 'pending'); renderCards(pen, dom.gridGestion); }
    }

    function renderCards(list, target = dom.grid) {
        if (!target) return; target.innerHTML = ''; if (!list || list.length === 0) { target.innerHTML = '<p>No hay resultados.</p>'; return; }
        list.forEach(pkg => {
            const card = document.createElement('div'); card.className = 'paquete-card';
            const noches = getNoches(pkg); const tarifa = parseFloat(pkg.tarifa) || 0;
            let statusTag = pkg.status === 'pending' ? `<span style="background-color:#ffeaa7; color:#d35400; padding:2px 8px; border-radius:10px; font-size:0.7em;">⏳ En Revisión</span>` : '';
            card.innerHTML = `<div class="card-clickable" onclick='window.openModal(${JSON.stringify(pkg).replace(/'/g, "&#39;")})'><div class="card-header"><div style="display:flex;justify-content:space-between;width:100%;"><div><h3 style="margin:0;color:#11173d;">${pkg.destino} ${statusTag}</h3></div>${noches>0?`<div style="background:#eef2f5;color:#11173d;padding:5px 10px;border-radius:12px;font-weight:bold;font-size:0.8em;">🌙 ${noches}</div>`:''}</div><div class="fecha">📅 Salida: ${formatDateAR(pkg.fecha_salida)}</div></div><div class="card-body"><div style="font-size:0.85em;color:#555;">${getSummaryIcons(pkg)}</div></div><div class="card-footer"><div><span style="background-color:#56DDE0;color:#11173d;padding:4px 12px;border-radius:20px;font-weight:600;font-size:0.75em;">${pkg.tipo_promo}</span></div><div><p class="precio-valor">${pkg.moneda} $${formatMoney(Math.round(tarifa/2))}</p></div></div></div>`;
            target.appendChild(card);
        });
    }

    // ===========================================================
    // 7. CALENDARIO & USUARIOS
    // ===========================================================
    window.openCalendarModal = async (dayKey, dayName) => {
        currentEditingDayKey = dayKey; if(dom.modalCalendarTitle) dom.modalCalendarTitle.innerText = `Editar ${dayName}`;
        try { const doc = await db.collection('config').doc('weekly_calendar').get(); const v = doc.exists ? (doc.data()[dayKey] || "") : ""; if(dom.calendarInput) dom.calendarInput.value = v; if(dom.modalCalendar) dom.modalCalendar.classList.add('active'); } catch(e) { window.showAlert("Error.", "error"); }
    };

    async function loadCalendar() {
        if (!dom.calendarContainer) return;
        const days = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes']; const todayIndex = new Date().getDay();
        try {
            const doc = await db.collection('config').doc('weekly_calendar').get(); const data = doc.exists ? doc.data() : {}; dom.calendarGrid.innerHTML = '';
            days.forEach((dayName, index) => {
                ...
                let editBtn = ''; if (userData && (userData.rol === 'admin' || userData.rol === 'editor')) { editBtn = `<button class="btn-edit-day" onclick="event.stopPropagation(); window.openCalendarModal('${dayKey}', '${dayName}')">✏️</button>`; }
                ...
                if (!content && (userData.rol === 'admin' || userData.rol === 'editor')) { card.style.cursor = 'pointer'; card.onclick = () => window.openCalendarModal(dayKey, dayName); }
            });
    ...
    async function loadUsersList() {
        ...
        try { const snap = await db.collection('usuarios').get(); list.innerHTML = ''; snap.forEach(doc => { const u = doc.data(); const li = document.createElement('div'); li.className = 'user-item'; li.innerHTML = `<span><b>${u.email}</b><br><small>${u.rol.toUpperCase()} - ${u.franquicia}</small></span><div style="display:flex; gap:5px;"><button class="btn btn-secundario" onclick="window.editUser('${u.email}', '${u.rol}', '${u.franquicia}')">✏️</button><button class="btn btn-secundario" onclick="window.confirmDeleteUser('${u.email}')">🗑️</button></div>`; list.appendChild(li); }); } catch (e) { list.innerHTML = 'Error.'; }
    }

    function configureUIByRole() {
        if(!userData) return; const rol = userData.rol;
        ...
        if(selectPromo) { selectPromo.innerHTML = rol === 'usuario' ? '<option value="Solo X Hoy">Solo X Hoy</option><option value="FEED">FEED (Requiere Aprobación)</option><option value="ADS">ADS (Requiere Aprobación)</option>' : '<option value="FEED">FEED</option><option value="Solo X Hoy">Solo X Hoy</option><option value="ADS">ADS</option>'; }
        updatePendingBadge();
    }
    ...
    async function softReload() {
        ...
        try {
            ...
            await loadCalendar(); await fetchAndLoadPackages(); showView('search');
        } catch(e) { console.error(e); }
        showLoader(false);
    }

    // ===========================================================
    // 8. LISTENERS Y EJECUCIÓN FINAL
    // ===========================================================
    auth.onAuthStateChanged(async (u) => {
        ...
        if (u) {
            ...
            } catch (e) { await window.showAlert("Error de conexión. Intente recargar."); }
    ...
    // Modales
    if(dom.modalClose) dom.modalClose.onclick = () => { dom.modal.classList.remove('active'); dom.modal.style.display='none'; };
    if(dom.btnToggleCalendar) dom.btnToggleCalendar.onclick = () => { dom.calendarGrid.classList.toggle('collapsed'); dom.btnToggleCalendar.classList.toggle('rotate-icon'); };
    if(dom.btnCloseCalendar) dom.btnCloseCalendar.onclick = () => dom.modalCalendar.classList.remove('active');
    if(dom.btnSaveCalendar) dom.btnSaveCalendar.onclick = async () => { const v = dom.calendarInput.value; if(!currentEditingDayKey) return; showLoader(true); try { await db.collection('config').doc('weekly_calendar').set({ [currentEditingDayKey]: v }, { merge: true }); dom.modalCalendar.classList.remove('active'); await loadCalendar(); showLoader(false); } catch(e) { window.showAlert("Error.", "error"); } };
