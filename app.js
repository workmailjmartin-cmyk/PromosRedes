document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA DE NOCHES (CORREGIDA: PRIORIDAD HOTEL) ---
    function getNoches(pkg) {
        ...
        // 1. Sumar noches de todos los hoteles
        let totalHotelNoches = 0;
        servicios.forEach(s => {
            if (s.tipo === 'hotel' && s.noches) {
                totalHotelNoches += parseInt(s.noches) || 0;
            }
        });
        if (totalHotelNoches > 0) return totalHotelNoches;

        // 2. Si no hay hotel, buscar Bus o Crucero
        ...
        // 3. Última opción: Calcular por fechas de salida/regreso
    ...
    async function uploadWithMutex(url, body) {
        ...
        if(!acquired) throw new Error("Sistema saturado. Intente en 1 minuto.");
    ...
    function generarTextoPresupuesto(pkg) {
        ...
        if (Array.isArray(servicios)) { servicios.forEach(s => { 
            if(s.tipo === 'aereo') { 
                ...
                // SEGURO: Solo Cobertura (Proveedor Oculto)
    ...
    function renderServiciosClienteHTML(rawJson) { 
        ...
        s.forEach(x=>{ 
            let i='🔹',t='',l=[]; 
            if(x.tipo==='aereo'){ 
                i='✈️';t='AÉREO'; l.push(`<b>${x.aerolinea}</b>`); l.push(`${formatDateAR(x.fecha_aereo)}${x.fecha_regreso?` - ${formatDateAR(x.fecha_regreso)}`:''}`); 
                let es = (x.escalas == 0 || x.escalas == '0') ? "Directo" : (x.escalas == 1 ? "1 Escala" : `${x.escalas} Escalas`); 
                l.push(`🔄 ${es} | 🧳 ${x.tipo_equipaje || '-'}`);
            } 
            else if(x.tipo==='hotel'){ 
                i='🏨';t='HOTEL'; 
                let stars = ''; if(x.hotel_estrellas) { for(let k=0; k<x.hotel_estrellas; k++) stars += '⭐'; }
                l.push(`<b>${x.hotel_nombre}</b> ${stars}`);
                l.push(`(${x.regimen})`); 
                
                let det = [];
                if(x.noches) det.push(`${x.noches} Noches`);
                if(x.checkin) det.push(`Ingreso: ${formatDateAR(x.checkin)}`);
                if(det.length > 0) l.push(det.join(' | '));

                if(x.hotel_link) l.push(`<a href="${x.hotel_link}" target="_blank" style="color:#ef5a1a; font-weight:bold;">📍 Ver Ubicación</a>`);
            } 
            else if(x.tipo==='traslado'){i='🚕';t='TRASLADO';l.push(`${x.tipo_trf}`);} 
            else if(x.tipo==='seguro'){
                i='🛡️';t='SEGURO';
                // SEGURO: Solo Cobertura
    ...
    function renderCostosProveedoresHTML(rawJson) { 
        ...
        s.forEach(x=>{ 
            ...
            let txt = `${x.proveedor||x.tipo}: $${x.costo}`;
            h+=`<li>${txt}</li>`; 
        }); 
        return h+'</ul>'; 
    }
        ...
        list.forEach(pkg => {
            ...
            card.innerHTML = `
                <div class="card-clickable">
                    ...
                    <div class="card-body"><div style="font-size:0.85em;color:#555;display:flex;flex-wrap:wrap;line-height:1.4;">${summaryIcons}</div></div>
                    <div class="card-footer"><div><span style="${bubbleStyle}">${pkg['tipo_promo']}</span></div><div><p class="precio-valor">${pkg['moneda']} $${formatMoney(Math.round(tarifaMostrar/2))}</p></div></div>
                </div>`;
    ...
    auth.onAuthStateChanged(async (u) => {
        ...
        if (u) {
            try {
                ...
                if (doc.exists) {
                    currentUser = u; userData = doc.data(); dom.loginContainer.style.display='none'; dom.appContainer.style.display='block';
    ...
    if(t==='aereo')m+=`<h4>✈️ Aéreo</h4><div class="form-group-row"><div class="form-group"><label>Aerolínea</label><input type="text" name="aerolinea" required></div><div class="form-group"><label>Ida</label><input type="date" name="fecha_aereo" required></div><div class="form-group"><label>Vuelta</label><input type="date" name="fecha_regreso"></div></div><div class="form-group-row"><div class="form-group"><label>Escalas</label>${crearContadorHTML('escalas',0)}</div><div class="form-group"><label>Equipaje</label><select name="tipo_equipaje"><option>Objeto Personal</option><option>Objeto Personal + Carry On</option><option>Objeto Personal + Bodega</option><option>Objeto Personal + Carry On + Bodega</option></select></div></div><div class="form-group-row"><div class="form-group"><label>Proveedor</label><input type="text" name="proveedor" required></div><div class="form-group"><label>Costo</label><input type="number" name="costo" class="input-costo" onchange="window.calcularTotal()" required></div></div>`;
        ...
        // HOTEL ACTUALIZADO: ESTRELLAS + LINK
        m+=`<h4>🏨 Hotel</h4>
    ...
    if(d){v.querySelectorAll('input, select, textarea').forEach(i=>{if(d[i.name]!==undefined){if(i.type==='checkbox'){i.checked=d[i.name];i.dispatchEvent(new Event('change'));}else if(i.type==='hidden'){i.parentElement.querySelector('.counter-value').innerText=d[i.name];i.value=d[i.name]; if(i.name.includes('estrellas')) window.setStars(id, d[i.name]);}else{i.value=d[i.name];if(i.name==='checkin'||i.name==='checkout')window.calcularNoches(id);}}});}}
